...
static int __parse_media_lib(const char *path)//解析媒体库
{
	...
	while(1)
	{
		...
		all_chns[ret->list.chnid] = ret;//把频道号作为全局数据的下标使用
		chn_nr++;//记录频道个数自增
	}
	closedir(dp);//关闭目录流
...
int mlib_read_chn_data(chnid_t chnid, void *buf, size_t size)
{
	...
	//char buf_str [size]=buf;
	int count;
	static int i = -1;
	//读取频道数据
	//读all_chns[chnid]->fd->读到EOF->读下一个文件
	if(all_chns[chnid]->fd==0){
	
		all_chns[chnid]->fd=__open_next(all_chns[chnid]->mp3_path,i++);
	    
	}
	count=read(all_chns[chnid]->fd,buf,size);
	if((count)==0)
	{
	...
	//读取频道数据
	//读all_chns[chnid]->fd->读到EOF->读下一个文件
	/*static int count=0;//实际读到的字节数
	static int cur_index=-1;//记录当前位置
	if(cur_index==-1)
	{
		all_chns[chnid]->fd=__open_next(all_chns[chnid]->mp3_path,cur_index);
	}
	//lseek(all_chns[chnid]->fd,count,SEEK_SET);
	if((count=read(all_chns[chnid]->fd,buf,size))<0)
	{
		...
		{
		count=0;
			cur_index = (cur_index + 1) % (all_chns[chnid]->mp3_path.gl_pathc);
			all_chns[chnid]->fd=__open_next(all_chns[chnid]->mp3_path,cur_index);
		}
	}
	count+=count;
