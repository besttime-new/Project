...
void NRF_RX_Mode(u8 rxAddr[][5], u8 channel, u8 recvLen[5],int nu)
{
	NRF_CE_LOW();	
	switch(nu)
	{
		case 0:SPI_NRF_WriteBuf(NRF_WRITE_REG+RX_ADDR_P0, rxAddr[0], RX_ADR_WIDTH);
		     SPI_NRF_WriteReg(NRF_WRITE_REG+RX_PW_P0, recvLen[0]);//????0??????? 
		     break;//??RX_ADDR_P0???
		case 1:SPI_NRF_WriteBuf(NRF_WRITE_REG+RX_ADDR_P1, rxAddr[1], RX_ADR_WIDTH);
		     SPI_NRF_WriteReg(NRF_WRITE_REG+RX_PW_P1, recvLen[1]);//????1??????? 
		     break;//??RX_ADDR_p1???
		case 2:SPI_NRF_WriteBuf(NRF_WRITE_REG+RX_ADDR_P2, rxAddr[2], 1);//??RX_ADDR_P2???
			   SPI_NRF_WriteReg(NRF_WRITE_REG+RX_PW_P2, recvLen[2]);//????2??????? 
		     break;
		case 3:SPI_NRF_WriteBuf(NRF_WRITE_REG+RX_ADDR_P3, rxAddr[3], 1);//??RX_ADDR_P3???
		     SPI_NRF_WriteReg(NRF_WRITE_REG+RX_PW_P3, recvLen[3]);//????3???????
         break;		
		case 4:SPI_NRF_WriteBuf(NRF_WRITE_REG+RX_ADDR_P4, rxAddr[4], 1);//??RX_ADDR_P4???
			   SPI_NRF_WriteReg(NRF_WRITE_REG+RX_PW_P4, recvLen[4]);//????4??????? 
		     break;
	}
	SPI_NRF_WriteReg(NRF_WRITE_REG+EN_AA, 0x1F);    //????0,1,2,3,4?????    
	SPI_NRF_WriteReg(NRF_WRITE_REG+EN_RXADDR, 0x1F);//????0,1,2,3,4?????    
	SPI_NRF_WriteReg(NRF_WRITE_REG+SETUP_AW, 0x03);//???5??
	SPI_NRF_WriteReg(NRF_WRITE_REG+RF_CH, channel); //??RF????      
