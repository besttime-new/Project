...
void delay_us(u32 nus)
{
	...
	do
	{
		midtime = SysTick->CTRL;												//接收CTRL寄存器的值并保存到midtime变量中
	}
...
* 入口参数：要延时的毫妙数
* 返回参数：无
* 说    明：nms:0~65535(因为上述的毫秒级的延时所传参数顶多1864毫秒)
****************************************************************************/
void delay_ms(u16 nms)
{	 	 
	u8 repeat = nms / 540;	//记录有多少个整的540ms
	u16 remain = nms % 540;	//记录有多少个不够540ms
	while(repeat)						//循环repat这么多次循环,相当于延时repat个540毫秒
	{
		delay_xms(540);				//延时540毫秒
		repeat--;
	}
	if(remain)delay_xms(remain);//继续延时remain个毫秒(不足540ms)
} 
