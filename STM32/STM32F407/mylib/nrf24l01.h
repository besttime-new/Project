...
*/
#include "stm32f4xx_conf.h"//包含了该头文件所有的外设头文件均已包含
#include "bitband.h"//包含位带文件
...
//这块也可以修改成位带操作
#define NRF_CSN_HIGH()          PBOut(12)=1  //片选拉高(不能操作设备)
#define NRF_CSN_LOW()      	    PBOut(12)=0  //片选拉低(可以操作设备)
#define NRF_CE_HIGH()	   	    PGOut(15)=1  //拉高CE(打开射频)
#define NRF_CE_LOW()	   	    PGOut(15)=0  //拉低CE(关闭射频)
#define NRF_Read_IRQ()		    PFIn(10)//读中断的管脚(无论是发送数据还是接受数据都是低电平)

#define NRF_MOSI_HIGH()			PCOut(3)=1   //MOSI拉高 
#define NRF_MOSI_LOW()			PCOut(3)=0   //MOSI拉低
#define NRF_CLK_HIGH()			PBOut(13)=1  //CLK拉高
#define NRF_CLK_LOW()			PBOut(13)=0  //CLK拉低
#define NRF_Read_MISO()			PCIn(2)      //读MISO(主入从出)
...
void NRF_RX_Mode(u8 rxAddr[][5], u8 channel, u8 recvLen[5]);//把Nrf设置为接收模式(自己的地址,自己的通道,自己接收数据长度)
/*void NRF_RX_Mode_1(u8 *rxAddr, u8 channel, u8 recvLen[5]);//把Nrf设置为接收模式(自己的地址,自己的通道,自己接收数据长度)
void NRF_RX_Mode_2(u8 *rxAddr, u8 channel, u8 recvLen[5]);//把Nrf设置为接收模式(自己的地址,自己的通道,自己接收数据长度)
void NRF_RX_Mode_3(u8 *rxAddr, u8 channel, u8 recvLen[5]);//把Nrf设置为接收模式(自己的地址,自己的通道,自己接收数据长度)
void NRF_RX_Mode_4(u8 *rxAddr, u8 channel, u8 recvLen[5]);//把Nrf设置为接收模式(自己的地址,自己的通道,自己接收数据长度)*/
