...
void SPI2_For_Nrf24l01(void) // 初始化NRF所使用到的SPI接口的管脚
{
	...
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;	  // 输出方式为推挽输出
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_12;		  // 选择了12号管脚
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; // 选择了50MHz输出速度
	GPIO_Init(GPIOB, &GPIO_InitStructure);			  // 按照上述配置初始化GPIOB组的管脚
	PBOut(12) = 1;									  // 默认是高电平(低电平有效)
	...
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;	  // 选择输出模式
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;	  // 输出方式为推挽输出
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_15;		  // 选择了15号管脚
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; // 选择了50MHz输出速度
	GPIO_Init(GPIOG, &GPIO_InitStructure);			  // 按照上述配置初始化GPIOG组的管脚
	PGOut(15) = 0;									  // 默认是低电平(既不发送也不接收)*/
	...
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;	  // 选择输出模式
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;	  // 输出方式为推挽输出
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_3;		  // 选择了3号管脚
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; // 选择了50MHz输出速度
	GPIO_Init(GPIOC, &GPIO_InitStructure);			  // 按照上述配置初始化GPIOC组的管脚
	...
	// SPI2_CLK PB13 OUT
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_13; // 选择了13号管脚
	GPIO_Init(GPIOB, &GPIO_InitStructure);	   // 按照上述配置初始化GPIOB组的管脚

	// MISO PC2 IN
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN;	 // 选择输入模式
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;		 // 选择了2号管脚
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL; // 选择了浮空输入
	GPIO_Init(GPIOC, &GPIO_InitStructure);			 // 按照上述配置初始化GPIOC组的管脚
	...
	// IRQ PF10 IN
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN; // 选择输入模式
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10;	 // 选择了10号管脚
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP; // 选择了上拉输入
	GPIO_Init(GPIOF, &GPIO_InitStructure);		 // 按照上述配置初始化GPIOF组的管脚
 ...
 * 调用  ：外部调用
 */
void NRF_RX_Mode(u8 rxAddr[][5], u8 channel, u8 recvLen[5])
{
	NRF_CE_LOW();
	SPI_NRF_WriteBuf(NRF_WRITE_REG+RX_ADDR_P0, rxAddr[0], RX_ADR_WIDTH);//设置RX_ADDR_P0的地址
	SPI_NRF_WriteBuf(NRF_WRITE_REG+RX_ADDR_P1, rxAddr[1], RX_ADR_WIDTH);//设置RX_ADDR_p1的地址
	SPI_NRF_WriteBuf(NRF_WRITE_REG+RX_ADDR_P2, rxAddr[2], 1);//设置RX_ADDR_P2的地址
	SPI_NRF_WriteBuf(NRF_WRITE_REG+RX_ADDR_P3, rxAddr[3], 1);//设置RX_ADDR_P3的地址
	SPI_NRF_WriteBuf(NRF_WRITE_REG+RX_ADDR_P4, rxAddr[4], 1);//设置RX_ADDR_P4的地址
	SPI_NRF_WriteReg(NRF_WRITE_REG+EN_AA, 0x1F);    //使能通道0,1,2,3,4的自动应答
	SPI_NRF_WriteReg(NRF_WRITE_REG+EN_RXADDR, 0x1F);//使能通道0,1,2,3,4的接收地址
	SPI_NRF_WriteReg(NRF_WRITE_REG+SETUP_AW, 0x03);//地址为5位的
	SPI_NRF_WriteReg(NRF_WRITE_REG+RF_CH, channel); //设置RF通信频率
	SPI_NRF_WriteReg(NRF_WRITE_REG+RX_PW_P0, recvLen[0]);//选择通道0的有效数据宽度 n
	SPI_NRF_WriteReg(NRF_WRITE_REG+RX_PW_P1, recvLen[1]);//选择通道1的有效数据宽度
	SPI_NRF_WriteReg(NRF_WRITE_REG+RX_PW_P2, recvLen[2]);//选择通道2的有效数据宽度
	SPI_NRF_WriteReg(NRF_WRITE_REG+RX_PW_P3, recvLen[3]);//选择通道3的有效数据宽度
	SPI_NRF_WriteReg(NRF_WRITE_REG+RX_PW_P4, recvLen[4]);//选择通道4的有效数据宽度
...
void NRF_TX_Mode(u8 *txAddr, u8 channel)
{
	...
	SPI_NRF_WriteBuf(NRF_WRITE_REG + RX_ADDR_P0, txAddr, RX_ADR_WIDTH); // 设置RX0节点地址,主要为了使能ACK
